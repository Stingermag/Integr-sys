	...
	function sortvak(typesort,counnt)
			{
					...
					xhr.open('GET' , 'sortnamevak.php?'+ body + '&&' + counts ,true);
...
"></a>
			...
			<div class="title">
				...
				<?php
						...
						if(strcasecmp($_SESSION["user"], "jobgiver") == 0 )
						{
						 echo "выставлять новые вакансии.";
						}
						?>
				</div>
...
<input type="button" onmousedown = "sortvak(1,document.getElementById('field'))" value="Сортировать по названию вакансий"/>
<input type="button" onmousedown = "sortvak(2,document.getElementById('field'))" value="Сортировать по опыту"/>

<script type="text/javascript"> sortvak(1,document.getElementById('field')); </script>	
...
$result = $mysqli->query("SELECT vacancy.idVacancy,vacancy.description,vacancy.expirience,jobgiver.fio FROM vacancy,jobgiver WHERE vacancy.Jobgiver_idJobgiver = jobgiver.idJobgiver"); 
	...
	echo '<table id ="tl"><tr><th>id</th><th>описание</th><th>опыт</th><th>работодатель</th><th> </th></tr>';
	while ($rows = $result->fetch_object()) 
	{ 
        echo "<tr><th>$rows->idVacancy</th><th>$rows->description</th><th>$rows->expirience</th><th>$rows->fio</th><th><a href=\"numvak.php?id=$rows->idVacancy\"> Подробнее</a></th></tr>";
    }
        echo '</table>';*/
if(strcasecmp($_SESSION["user"], "jobgiver") == 0 )
{
	echo "<a href=\"addvak.php\">Добавить вакансию</a>";
}
