		...
		<div class="container">
			<div class="title">
				...
				<div>
					...
					$jober = $_SESSION['jober'];

					$result = $mysqli->query("

SELECT `vacancy`.`idVacancy` , `vacancy`.description as vacdisc, `request`.idRequest, `resume`.description as resdisc, `students`.fio, `students`.idStudent
FROM `vacancy` 
    ...
    WHERE vacancy.Jobgiver_idJobgiver = $jober");

					echo '<table id ="tl"><tr><th>id вакансии</th><th>описание вакансии</th><th>номер заявки</th><th>описание резюме</th><th>фио студента</th><th>ссылка</th><th>подтверждение</th></tr>';
        ...
        <th>$rows->resdisc</th>
        <th>$rows->fio</th>
        <th><a href=\"numrez.php?id=$rows->idStudent\"> $rows->idStudent</a></th>";
						if ("$rows->idRequest" != null)
							echo "<th><a href=\"applyreq.php?id=$rows->idRequest\">Принять заявку</a></th></tr>";
						else {
							echo "</tr>";
						}
					}
